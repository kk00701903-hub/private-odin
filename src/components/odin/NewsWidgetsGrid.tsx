  ...
  return (
    <li className="group flex flex-col gap-0.5 py-1.5 border-b border-border/30 last:border-0 cursor-pointer hover:bg-muted/20 rounded px-1 -mx-1 transition-colors">
      <div className="flex items-start gap-1.5">
        {item.tag && (
          <span className={`mt-0.5 inline-flex px-1.5 py-0 text-[11px] font-mono font-semibold rounded border flex-shrink-0 ${TAG_COLOR[item.tagColor ?? 'cyan']}`}>
            {item.tag}
          </span>
        )}
      ...
      <div className="flex items-center gap-1.5 pl-0.5">
        ...
        <span className="text-[12px] text-muted-foreground/70 font-mono">{item.time}</span>
      </div>
    </li>
  )
}
  ...
  return (
    <div className="flex items-center justify-between py-1 border-b border-border/30 last:border-0">
      <div>
        ...
        <p className="text-[12px] text-muted-foreground font-mono">{q.ticker}</p>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center gap-2 px-3 py-2 border-b bg-muted/20 ${headerColor[accentColor]}`}>
        ...
        <span className="text-[12px] font-mono font-semibold uppercase tracking-widest">{title}</span>
