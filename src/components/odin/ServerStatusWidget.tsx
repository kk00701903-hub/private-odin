    ...
    <div className="group rounded-md border border-border/50 bg-muted/20 px-3 py-2 hover:border-primary/40 hover:bg-muted/40 transition-all duration-200">
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="hidden group-hover:inline text-[10px] text-muted-foreground">VM {vm.id}</span>
        </div>
        <span className="text-[10px] text-muted-foreground font-mono">{vm.role}</span>
      </div>
      ...
      {vm.status === 'running' && (
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono text-primary w-8 text-right">{vm.cpu}%</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-muted-foreground w-8">MEM</span>
            <GaugeBar value={vm.mem} max={vm.memMax} color={memColor} />
            <span className="text-[10px] font-mono text-accent w-8 text-right">{vm.mem}G</span>
          </div>
        </div>
      )}

      {vm.status === 'stopped' && (
        <p className="text-[10px] text-muted-foreground font-mono mt-0.5">[ OFFLINE — 23:00 자동 종료 예약 ]</p>
      )}
    </div>
  )
}
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border/60 bg-muted/20">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-[10px] font-mono font-bold ${isOnline ? 'text-chart-3' : 'text-destructive'}`}>
      ...
      <div className="px-3 py-2 border-b border-border/40 bg-muted/10">
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          <div>
            ...
            <span className="text-[10px] font-mono text-primary">{node.cpu}%</span>
          </div>
          <div>
            <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-[10px] font-mono text-accent">{node.memUsed}/{node.memTotal}G</span>
          </div>
          <div>
            <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-[10px] font-mono text-chart-3">{node.storage}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-mono">Virtual Machines</span>
        </div>
        {VM_LIST.map((vm) => (
      ...
      <div className="px-3 py-1.5 border-t border-border/40 bg-muted/10 flex items-center gap-1">
        ...
        <span className="text-[10px] font-mono text-muted-foreground">VM 102 AI Core — 활성 추론 중</span>
